...
<script type="text/javascript">
	...
	function fncGetSaleList(currentPage) {
		...
		$('form').attr("method", "POST").attr("action",
				"/purchase/listSale?menu=${param.menu}").submit()
	}
</script>
</head>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<c:forEach var="purchase" items="${list}">
					...
					<tr class="ct_list_pop">
						...
						<td></td>
						<td align="left">
						<c:if test="${fn:trim(purchase.tranCode) eq '1' }">구매완료
						<b data-tranNo="${purchase.tranNo}">배송하기</b></c:if>
						<c:if test="${fn:trim(purchase.tranCode) eq '2' }">배송중</c:if> 
