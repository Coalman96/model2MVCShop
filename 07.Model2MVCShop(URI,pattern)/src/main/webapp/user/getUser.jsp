	...
	<script type="text/javascript">
		...
		//==> 추가된부분 : "수정" "확인"  Event 연결 및 처리
		 $(function() {
			...
			// $( "button.btn-outline-secondary:contains('확인')" ).on("click" , function() {
				//Debug..
				//alert(  $( "td.ct_btn01:contains('확인')" ).html() );
				//history.go(-1);
			//});
			
			 $( "button.btn-outline-primary:contains('수정')" ).on("click" , function() {
					//Debug..
					//alert(  $( "td.ct_btn01:contains('수정')" ).html() );
					//self.location = "/user/updateUser?userId=${user.userId}"
				 // AJAX 요청을 통해 다른 JSP 페이지 내용을 가져옵니다.
			        $.ajax({
			            url: "/user/updateUser?userId=${user.userId}", // 다음 JSP 페이지의 경로
...
<section style="background-color: #f4f5f7; width:800px; height:550px;">
  ...
  <div class="d-flex justify-content-center align-items-center gap-5">
    <button type="button" class="btn btn-outline-primary px-6">수정</button>
